...
void FAnimNode_AdjustFootToZ::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	FMath::SinCos(&OffsetSin, &OffsetCos, PelvisCSTransform.Rotator().Yaw);

	JointTargetLocation.Y += (JointForwardOffset * OffsetSin);
	JointTargetLocation.X += (JointForwardOffset * OffsetCos);
